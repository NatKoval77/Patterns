...
class Student_short(id: Int, val surnameIn: String, git: String, val contact: String)
    ...
    override fun optValidate() {
        ...
        super.optValidate()
        if (!StudentBase.isValidTg(contact) && !StudentBase.isValidEmail(contact) && !StudentBase.isValidPhone(contact))
    ...
    fun writeShort() {
        var what = ""
        if (StudentBase.isValidTg(contact))
            what = "tg"
        else if (StudentBase.isValidEmail(contact))
            what = "email"
        else if (StudentBase.isValidPhone(contact))
            what = "phone"
        println(
